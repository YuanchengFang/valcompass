...
struct MethodologyView: View {
    ...
    private static let topics: [Topic] = [
        ...
        """),
        Topic(id: 3, title: "其他视角（辅助指标）", body: """
        主分数之外，部分标的提供不并入主分数的辅助视角：

        股债性价比（ERP）= 盈利收益率（1/PE × 100）− 10 年期国债收益率，衡量股票相对债券的便宜程度。注意方向：ERP 分位越高越便宜，与主分数正好相反。局限：受利率 regime 影响大，长期低利率会系统性抬高 ERP，历史分位不等于未来。

        Shiller CAPE 分位（仅标普500）：用 10 年均通胀调整盈利平滑盈利周期，再看当前 CAPE 在近 10 年月度序列中的位置。分位越高越贵，与主分数同向。

        股息率分位（仅标普500）：当前股息率在近 10 年月度序列中的位置。分位越高越便宜，方向与主分数相反。

        三者都只描述相对历史的位置，均不用于买卖信号。
        """),
        Topic(id: 4, title: "置信度", body: """
        置信度反映历史数据的充分程度：覆盖 8 年以上为「高」，3–8 年为「中」，不足 3 年为「低」。月度数据会进一步降低精度。辅助指标统一标「中」：它们的输入（利率、平滑盈利、分红）受结构性变化影响更大。数据不足时，应用宁可不显示分数，也不会用假设数据填充。
        """),
        Topic(id: 5, title: "ETF 与溢折价", body: """
        ETF 的估值分数直接采用其底层指数的评分，因为 ETF 本身没有独立的估值含义。唯一的例外是黄金ETF：黄金没有盈利，PE 不适用，因此用价格位置法评估 ETF 自身市价。

        溢折价是另一个维度：溢折价率 = 市价 ÷ 单位净值 − 1，反映交易价格相对基金净值的偏离。QDII 基金（如纳指ETF、标普500ETF）净值滞后 1–2 个交易日，此时溢折价仅供参考。即使底层资产估值合理，高溢价买入仍可能承受溢价回落的损失。
        """),
        Topic(id: 6, title: "名单如何选出", body: """
        全名单 27 个可见标的，选择标准是：流动性与认知度高，方便长期跟踪；横跨 A股、港股、美股三个市场；指数与 ETF 兼顾，既有观察基准也有可交易载体。ETF 组只保留指数组无法提供的敞口或信息（行业主题、黄金等商品、QDII 跨境与有意义的溢折价）：底层已在指数组里的宽基 ETF 不重复收录，其唯一增量信息溢折价常年接近 0。另有 10 个隐藏底层指数（红利、银行、医疗、光伏、半导体、创业板指、军工、消费、新能车、有色金属），只取数算分供 ETF 引用，不在总览出现。

        名单受免费公开数据源覆盖能力约束（腾讯行情、新浪美股、中证指数、天天基金、东方财富、multpl.com），并非全市场优选，也不代表推荐其中任何标的。
        """),
        Topic(id: 7, title: "数据与时效", body: """
